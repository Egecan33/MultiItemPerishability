...
Persistence so reruns don't re-check the same rows:
  - checked_ids.txt : every row id we fully processed (feasible OR mutated)
  - cursor.txt      : last_seen_id for keyset pagination
...
Env vars (optional):
  ...
  START_AFTER_ID          (keyset resume; process rows with id > START_AFTER_ID)
...
START_AFTER_ID = os.getenv("START_AFTER_ID")
...
def save_cursor(path: Path, inst_id: str) -> None:
...
def fetch_page_with_retry(
    sb: Client, last_seen_id: Optional[str], page_size: int
...
def update_with_verify(
    ...
    """Write JSONB and (optionally) verify by re-reading the row.

    We don't call .select() after update because SyncFilterRequestBuilder doesn't support it.
    """
    if DRY_RUN:
        return True
    ...
    for attempt in range(UPDATE_RETRIES):
        try:
            # WRITE (id match ensures we touch only the intended row)
            sb.table(TABLE_NAME).update({"data": new_data}).eq("id", inst_id).execute()

            if not VERIFY_AFTER_UPDATE:
                return True

            # READ-BACK (separate SELECT)
            stored = _fetch_data_by_id(sb, inst_id)
            if stored is None:
                print(f"[{inst_id}] Verify read returned no row.", flush=True)
            else:
                # Light verification: only check demands up to the last zeroed t
                if must_have_zeroed_up_to_t is None:
                    # fall back to normalized full-JSON check (best effort)
                    if _normalize_json(stored) == _normalize_json(new_data):
                        return True
                else:
                    ok = _verify_zeroed_prefix(stored, must_have_zeroed_up_to_t)
                    if ok:
                        return True
                    else:
                        print(
                            f"[{inst_id}] Post-update verify mismatch on demands up to t={must_have_zeroed_up_to_t}.",
                            flush=True,
                        )
...
def run_solver(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        summary, _ = _solve_base(
            ...
            mip_gap=0.0,
            stop_on_first_gap=True,  # silently ignored by non-patched solver
        )
    except TypeError:
        summary, _ = _solve_base(
...
def infer_horizon(data: Dict[str, Any]) -> int:
    ...
    try:
        ...
        pass
    # Otherwise infer from demand lengths (matches your real JSON layout)
    items = (data or {}).get("items") or {}
    T = 0
    for _, it in items.items():
        dem = it.get("demand", [])
...
# -------------------- Row processing --------------------
def process_row(row: Dict[str, Any], sb: Client) -> Tuple[bool, bool, str]:
    """
    Returns (was_mutated, was_checked, id_str)
    """
...
def main() -> None:
    ...
    start_cursor = START_AFTER_ID or load_cursor(CURSOR_PATH)
    ...
    processed = 0
    mutated = 0
    last_seen_id = start_cursor

    while True:
        rows = fetch_page_with_retry(sb, last_seen_id, PAGE_SIZE)
        if rows is None:
        ...
        for row in rows:
            ...
            if row_id in checked_ids:
                last_seen_id = row_id
                save_cursor(CURSOR_PATH, last_seen_id)
                continue

            changed, was_checked, id_str = process_row(row, sb)
            processed += 1
            last_seen_id = id_str
            save_cursor(CURSOR_PATH, last_seen_id)
    ...
    print(f"Cursor file:      {CURSOR_PATH.resolve()}", flush=True)
    print("=============================================\n", flush=True)
