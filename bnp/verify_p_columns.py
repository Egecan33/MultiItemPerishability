...
This script constructs the 4 exact columns from the pr's diagram
...
def main():
    ...
    for col in columns:
        ...
        print(
            f"  Setups = {sum(col['y'])} at periods {[t for t in range(T) if col['y'][t] == 1]}"
        )
    ...
    m1.setObjective(
        gp.quicksum(columns[i]["total_cost"] * lam[i] for i in range(4)), GRB.MINIMIZE
    )
    m1.optimize()
    ...
    # The pr's formulation links aggregate X to specific values
    print("\n" + "=" * 70)
    ...
    m2.setObjective(
        gp.quicksum(columns[i]["total_cost"] * lam2[i] for i in range(4)), GRB.MINIMIZE
    )
    m2.optimize()
    ...
    # Now let's try what the pr's LP structure suggests:
    # The constraints force specific X values at key periods
    print("\n" + "=" * 70)
    print("LP WITH INTENDED STRUCTURE")
    print("=" * 70)
    ...
    m3.addConstr(
        49 * lam3[0] + 153 * lam3[1] + 49 * lam3[2] + 153 * lam3[3] == 73, "x3"
    )
    ...
    m3.addConstr(
        17 * lam3[0] + 17 * lam3[1] + 118 * lam3[2] + 118 * lam3[3] == 38, "x6"
    )
    ...
    # Objective: minimize cost
    m3.setObjective(
        gp.quicksum(columns[i]["total_cost"] * lam3[i] for i in range(4)), GRB.MINIMIZE
    )
    m3.optimize()
    ...
    if m3.Status == GRB.OPTIMAL:
        ...
        total_x = sum(
            sum(columns[i]["x"][t] * lam3[i].X for i in range(4)) for t in range(T)
        )
        ...
        for i in range(4):
            ...
            print(
                f"  λ[{i+1}]: got {lam3[i].X:.6f}, expected {expected_lam[i]:.6f} {status}"
            )
    else:
    ...
    # Objective: minimize cost
    m4.setObjective(
        gp.quicksum(columns[i]["total_cost"] * lam4[i] for i in range(4)), GRB.MINIMIZE
    )
    m4.optimize()
    ...
    if m4.Status == GRB.OPTIMAL:
        ...
        for i in range(4):
            if lam4[i].X > 1e-6:
                print(
                    f"  λ[{i+1}] = {lam4[i].X:.6f} (cost={columns[i]['total_cost']:.2f})"
                )
