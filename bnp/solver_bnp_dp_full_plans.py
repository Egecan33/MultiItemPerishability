...
def price_zio_column_dp(
    ...
    tau_y and tau_z duals give "discounts" to columns that satisfy forced constraints,
    ...
    if not demand_periods:
        ...
        # Add tau_y discounts for forced Y periods
        tau_y_discount = sum(
            tau_y.get(t, 0.0) for t in forced_y if t not in forbidden_y
        )
        rc = dummy_cost - mu - tau_y_discount
        if rc < -EPS:
            y = {t: 1 for t in forced_y if t not in forbidden_y}
            z = {}
            for t, u in forced_z:
    ...
    for i in range(n):
        ...
        for t in range(u_start + 1):
            ...
            for j in range(i, n):
                ...
                # tau_y discount: if block at period t and t is forced, get discount
                tau_y_discount = tau_y.get(t, 0.0) if t in forced_y else 0.0
                ...
                reduced = actual - rho_t * total_prod - tau_y_discount - tau_z_discount
    ...
    for rc_path, actual_path, path in F[n]:
        ...
        # Add forced_y dummy setup costs (with tau_y discount)
        dummy_setup_cost = 0.0
        dummy_tau_y_discount = 0.0
        for t in forced_y:
            if t not in production_periods and t not in forbidden_y:
                dummy_setup_cost += setup[t]
                dummy_tau_y_discount += tau_y.get(t, 0.0)
...
def solve_rmp_with_duals(
    ...
    Dict[Tuple[int, int], float],  # tau_y duals
    Dict[Tuple[int, int, int], float],  # tau_z duals
]:
    """
    Solve the RMP and return (objective, mu_duals, rho_duals, lambda_values, tau_y, tau_z).
    ...
    τ_y and τ_z duals give "discounts" for pricing to generate required columns.
    ...
    tau_y = {key: con.Pi for key, con in y_fix_cons.items()}
    ...
    return m.ObjVal, mu, rho, lam_vals, tau_y, tau_z
...
def column_generation_loop(
    ...
    for cg_iter in range(max_cg_iters):
        ...
        obj, mu, rho, lam_vals, tau_y, tau_z = solve_rmp_with_duals(
        ...
        for i, item_data in items.items():
            ...
            tau_y_item = {t: tau_y.get((i, t), 0.0) for t in forced}
    ...
    obj, mu, rho, lam_vals, _, _ = solve_rmp_with_duals(
        items, columns, capacity, T, forced_y, forbidden_y, forced_z, forbidden_z
    )
