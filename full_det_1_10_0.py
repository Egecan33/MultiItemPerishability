...
dual stabilisation, order/no-order branching on (item,period), and new options:
...
# NEW: generation knobs (only used when RANDOMIZE=True)
USE_SETUP_SEQ = True  # generate time-varying setup costs s_{i,t}
USE_ITEM_CAP_SEQ = True  # generate per-item capacity sequences cap_{i,t}
WAREHOUSE_CAPACITY = None  # e.g., 50000; None → no warehouse cap rows

# --- Setup control knobs (optional) -----------------------------------------
# If you set any of these, they override whatever build_lot() created.
# Use ONE or many; later entries override earlier ones (JSON → scalar → sequence).
SETUP_JSON_OVERRIDES = None  # e.g., "setup_sequences.json"
SETUP_SCALAR_OVERRIDES = {  # e.g., force constant setup for some items
    # 0: 150.0,
    # 3: 95.0,
}
SETUP_SEQUENCE_OVERRIDES = {  # per-item full length-T sequences
    # 1: [120.0]*365,
    # 2: [100.0 + 10.0*((t%30)<5) for t in range(365)],  # short promo windows
}
...
def price_shortest_path(
    ...
    c_var: float,
    h: float,
    setup_seq: Union[float, List[float]],  # NEW: setup can be scalar or list
    mu_eff: List[float],  # NEW: effective duals per t (global+item+warehouse)
    pi: float,
    m_seq: List[int] | None,
    k_max: int,
    ...
    for t in range(T - 1, -1, -1):
        ...
        for u in range(t, u_cap + 1):
            ...
            # reduced arc cost: true arc cost minus mu_eff[t]*qty
            red_arc = (
...
class MasterModel:
    ...
    def __init__(
        ...
        # warehouse capacity rows (optional) — define for u=0..T-2
    ...
    def add_pattern(self, i: int, cost: float, q: List[int]):
        ...
        if self.whcap_constr:
            inv_tail = self._inv_tail_vector(i, q)  # length T-1
    ...
    def optimize(self):
        ...
        # duals
        pi = {i: self.sel_constr[i].Pi for i in self.items}  # selection
        mu_global = [c.Pi for c in self.cap_constr]  # global capacity
...
class BranchPrice:
    def __init__(
        ...
        self.prev_mu_base = [
            0.0
        ] * self.T  # stabilised base dual (global + warehouse tail)
        ...
        BIG_M = 1e6
        for i in self.items:
            q = self.dem[i][:]  # produce exactly demand each period
    ...
    def price_mip_perishable(
        ...
        s_seq = self.setup[item_id]  # scalar or list
        ...
        # objective: true cost − mu_eff[s]*q[s] − π_i
    ...
    def price_with_growth(
        ...
        """Build effective mu for item i and try SP → DP → MIP → Heuristic."""
        # Effective per-period dual for item i
    ...
    # --- FEFO DP with s_{t} and mu_eff[t] ---
    def price_fefo_dp(self, item_id: int, mu_eff: List[float], pi_i: float):
        ...
        for a in range(T - 1, -1, -1):
            ...
            for t in range(0, a + 1):
                ...
                if R_tau < a:
                    continue
                # A_t(a) + B_t * U_tau, valid for U in [U[a], U[R_tau+1]]
    ...
    def fallback_heuristic_pattern(self, i: int, mu_eff: List[float], pi_i: float):
        D = self.dem[i]
        T = len(D)
        q = [max(0, d) for d in D]  # naive: order-on-demand
        s_seq = self.setup[i]
        setup_cost = sum(
            (s_seq[t] if isinstance(s_seq, list) else float(s_seq))
            for t in range(T)
        ...
        true = setup_cost + self.c_var[i] * sum(q)  # ignore holding
    ...
    def column_generation(self):
        while True:
            ...
            # Fold warehouse duals: contribution of q[t] to invTail rows is sum_{u=t}^{T-2} 1 * nu[u]
            tail = [0.0] * self.T
            for t in range(self.T):
                tail[t] = sum(nu_wh[u] for u in range(t, self.T - 1)) if nu_wh else 0.0

            # Base dual per t (global + warehouse tail)
    ...
    def branch_child(self, i, t, fix, best, best_sol, best_patterns):
        ...
        # Shallow-copy immutable / shared data structures
...
class Lot:
    ...
    warehouse_capacity: Optional[float] = None  # NEW
    ...
    def to_mip_json(self, path: str | Path, *, indent: int = 2) -> None:
        """
        Write the compact/extended MIP schema the solver expects:
        - period
        - optional manual_capacity
        - optional warehouse_capacity
        - items: demand, setup (scalar or list), c_var, h, b_var, shelf_seq, optional cap_seq
        """
        path = Path(path)
        path.parent.mkdir(parents=True, exist_ok=True)
        payload = {
...
def build_lot(
    ...
    use_setup_seq: bool = False,
    use_item_cap_seq: bool = False,
    ...
    for rec in specs:
        # supported: (idx, stp, b, c, h) or (idx, stp, b, c, h, (m_lo, m_hi))
        if len(rec) == 5:
        ...
        # time-varying setups (gentle seasonal oscillation ±10%)
        if use_setup_seq:
            s_seq = [
                float(stp)
                * (0.9 + 0.2 * (0.5 + 0.5 * math.sin(2 * math.pi * t / 30.0)))
                for t in range(period)
            ]
            setup_val: Union[float, List[float]] = s_seq
        else:
            setup_val = float(stp)

        # per-item capacities: generous (not binding by default)
        cap_seq = None
        if use_item_cap_seq:
            cap_seq = [max(demand[t] + 50, int(1.2 * demand[t])) for t in range(period)]

        lot.items[idx] = Item(idx, demand, setup_val, b, c, hold, shelf_seq, cap_seq)
    return lot
...
def apply_setup_overrides(lot: "Lot", period: int) -> None:
    """
    Priority: JSON file (if given) → SETUP_SCALAR_OVERRIDES → SETUP_SEQUENCE_OVERRIDES.
    JSON format example:
      {
        "0": [120.0, 120.0, ..., 120.0],   // length == period
        "5": [100.0, 101.0, ..., 130.0]
      }
    """
    # 1) JSON sequences
...
if __name__ == "__main__":
    ...
    if (not RANDOMIZE) and INSTANCE_PATH.exists():
        ...
        lot = build_lot(
            ...
            use_setup_seq=USE_SETUP_SEQ,
            use_item_cap_seq=USE_ITEM_CAP_SEQ,
            warehouse_capacity=WAREHOUSE_CAPACITY,
        )
        ...
        # >>> NEW: take control of setups from here <<<
        apply_setup_overrides(lot, period)

        # Write instances for both B&P and MIP consumers
        lot.to_mip_json(INSTANCE_PATH)
        lot.to_mip_json(Path.cwd() / "last_instance.json")
        print(f"[INFO] Generated MIP instance → {INSTANCE_PATH}")
        print(f"[INFO] Copied MIP instance   → {Path.cwd() / 'last_instance.json'}")
    run_and_save(lot)
