...
class Lot:
    ...
    def kmax(self) -> Dict[int, int]:
        ...
        return {
            i: sum(it.demand) + buffer  # allow one big order covering everything
...
if __name__ == "__main__":
    ...
    # ---- basic switches -------------------------------------------------
    RANDOMIZE = False  # → False to re-use the cached instance
    ...
    # 2️⃣  Otherwise build a fresh instance and overwrite the cache
        ...
        specs = [
            ...
            (0, 127.5, 5.0, 2.0, 0.4, 20),
            (1, 29.0, 5.0, 3.0, 0.6, 4),
            (2, 50.0, 5.0, 1.0, 0.2, 10),
            (3, 100.0, 5.0, 2.0, 0.5, 15),
            (4, 75.0, 5.0, 1.5, 0.3, 12),
            (5, 80.0, 5.0, 2.5, 0.7, 18),
            (6, 60.0, 5.0, 1.2, 0.4, 8),
            (7, 90.0, 5.0, 2.8, 0.6, 16),
        ]
        period = 30  # number of periods in the lot
        manual_caps = [1000] * period if USE_MANUAL_CAPACITY else None
        lot = build_lot(
            period=period,
            lb_dem=1,
            ub_dem=10,
