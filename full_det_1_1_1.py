...
class BranchPrice:
    ...
    def column_generation(self):
        while True:
            ...
            if res is None:
                ...
                return float("inf"), None  # instead of just float("inf")
            obj, pi, mu, lam = res
            print(
    ...
    def branch_and_price(self, best=float("inf"), best_sol=None):
        bound, _ = self.column_generation()
        if bound >= best - 1e-6:
        ...
        while True:
            ...
            if q is None:
                if k >= HARD_KMAX_CAP:
                    ...
                    return float("inf"), float("inf"), None  # ← NO empty list!
                k *= 2
                continue
...
if __name__ == "__main__":
    ...
    # 2️⃣  Otherwise build a fresh instance and overwrite the cache
        ...
        manual_caps = [20] * 12 if USE_MANUAL_CAPACITY else None
        lot = build_lot(
            period=12,
            lb_dem=1,
            ub_dem=10,
