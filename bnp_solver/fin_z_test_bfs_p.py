...
def solve_branch_and_price(
    ...
    if is_integer(z_vals, eps):
        ...
        _, opt_rmp, _, _ = solve_node_with_column_generation(items, capacity, root)
        return root_lb, root_lb, 1, root, z_vals, opt_rmp
    ...
    while queue and nodes_explored < max_nodes:
        node, parent_z = queue.popleft()  # FIFO: BFS order
        ...
        if node.node_id != 0:
            ...
            if is_integer(z_vals, eps):
                ...
                if best_ub is None or lb < best_ub - eps:
                    best_ub = lb
                    print(" ★ NEW INCUMBENT!")
                    stats.node_explored(node, incumbent_improved=True)
                    opt_node = node
                    opt_z = z_vals
                    _, opt_rmp, _, _ = solve_node_with_column_generation(
                        items, capacity, node
                    )
                else:
                    print()
        ...
        if can_be_one:
            ...
            if sig not in seen_signatures:
                ...
                node_counter += 1
                children.append((right, z_vals))

        for child in children:  # enqueue FIFO
            queue.append(child)
