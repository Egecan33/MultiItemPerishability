...
class BranchPrice:
    ...
    def price_fefo_dp(self, item_id: int, mu: List[float], pi_i: float):
        ...
        for a in range(T - 1, -1, -1):
            root = self._LiChao(Xmin, Xmax)  # <-- FIXED: use the nested class via self
            Ua, Va = U[a], V[a]
            ...
            for i in self.items:
                ...
                if q is not None and rc < -1e-6:
                    ...
                    if len(self.master.lambda_vars[i]) > before:
                        ...
                        print(
                            f"    ↳ duplicate col for item {i}  rc={rc:.2f} (skipped)"
                        )
            if not added:
                break
...
# ────────────────────────────────────────────────────────────────────────────
def detailed_exec_rep(bp: BranchPrice, sol):
    ...
    for i in bp.items:
        ...
        try:
            ...
            continue
        pat = bp.master.patterns[i][sel_idx]
        orders = pat["q"]
        D = bp.dem[i]
        m_seq = bp.mseq[i]
        ...
        if sol is None:
            ...
            for i in bp.items:
                try:
                    ...
                    print(
                        f"Item {i}: pattern {sel}, y={patterns_i[sel].get('y')}, q={patterns_i[sel]['q']}"
                    )
...
if __name__ == "__main__":
    ...
    if (not RANDOMIZE) and INSTANCE_PATH.exists():
        ...
        specs = [
            ...
            (5, 60.0, 5.0, 3.5, 0.7, (3, 5)),
            (6, 80.0, 5.0, 1.5, 0.2, (3, 5)),
            (7, 90.0, 5.0, 2.0, 0.3, (3, 5)),
            (8, 110.0, 5.0, 2.8, 0.4, (3, 5)),
            (9, 120.0, 5.0, 3.2, 0.6, (3, 5)),
            (10, 130.0, 5.0, 4.0, 0.8, (3, 5)),
            (11, 140.0, 5.0, 2.2, 0.3, (3, 5)),
            (12, 150.0, 5.0, 1.8, 0.4, (3, 5)),
            (13, 160.0, 5.0, 2.6, 0.5, (3, 5)),
        ]
        period = 1000
        manual_caps = [20000] * period if USE_MANUAL_CAPACITY else None
        lot = build_lot(
            period=period,
            lb_dem=1,
            ub_dem=200,
